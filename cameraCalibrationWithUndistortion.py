...
def calibrate_camera_with_undistortion(image_dir, output_file, checkerboard_size=(7, 10), show_display=True):
    """
    Performs camera calibration using checkerboard images, saves results to a JSON file,
    and displays undistorted images.
    ...
    Args:
        ...
        show_display (bool): Whether to display the undistorted image.
    """
    print(f"Starting camera calibration with undistortion...")
    ...
    if show_display and images:
        try:
            img = cv2.imread(images[0])
            if img is None:
                print("Warning: Could not read first image for undistortion demonstration.")
                return
            ...
            # Method 2 to undistort the image (alternative)
            # mapx, mapy = cv2.initUndistortRectifyMap(mtx, dist, None, newcameramtx, (w, h), 5)
            # dst = cv2.remap(img, mapx, mapy, cv2.INTER_LINEAR)

            print("\nDisplaying undistorted image. Press any key to continue...")
            ...
            print(f"Error displaying undistorted image: {e}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Camera Calibration with Undistortion Script")
    parser.add_argument(
        "-i", "--image_dir",
        type=str,
        default="./images",
    ...
    parser.add_argument(
        "--no-display",
        action="store_true",
        help="Skip displaying undistorted image"
    )
    ...
    calibrate_camera_with_undistortion(args.image_dir, args.output_file, checkerboard_dims, show_display)